...
for sourcefile in sourcefiles:
	...
	for record in root.findall('.//shop_record'):
		record_data = {
			...
			'interchange_fee': float(record.find('interchange_fee').text.replace(',', '.')),
			'scheme_fee': float(record.find('scheme_fee').text.replace(',', '.')),
			'acquirer_fee': float(record.find('acquirer_fee').text.replace(',', '.')),
			'total_comission': float(record.find('total_commission').text.replace(',', '.')),
			'card_segment': record.find('card_segment').text,
			'card_region': record.find('card_region').text,
			'card_brand': record.find('card_brand').text,
			'card_type': record.find('card_type').text
	}
	...
	wb['poslist']['M1']='=SUM(H:H)'  #összegzés a poslist lapon
